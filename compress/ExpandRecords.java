...
/**
 * Use CompressionFactory to de-compress (expand) individual records from one data set
 * to another data set. 
 * 
...
public class ExpandRecords {
	...
	public static void main(java.lang.String[] args) throws Exception {
		...
		// get a BufferCompressor from the default
		try {
			...
			// Read, expand and write each record.
			// BufferCompressor.expandBuffer() is used, as it will ignore
			// a partial unused byte at the end of the compressed data.
			while ((nRead = reader.read(inBuf)) >= 0) {
				elen = comp.expandBuffer(outBuf, 0, inBuf, 0, nRead);
				if (elen < 0) {
			...
			secs = secs / 1000;
			System.out.println("Expanded "+nRecs+" records; bytes read/written="
								+nTotalRead+"/"+nTotalWritten+
								" in "+secs+" seconds");
		
		} finally {
			// Clean up after completion or if something goes wrong
			if (reader != null) {
